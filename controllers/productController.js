    ...
    try {
        ...
        if (Object.values(req.body).some(
            ...
            if (productData == null) {
                ...
                await product.save();
                const categoryData = await Category.find();
                res.render("admin/addProduct", { categoryData , message: "Product added succesfuly" });
            } else {
...
const loadEditProduct = async (req, res) => {
    try {
        ...
        console.log(images,"helooooooooooo");
        if (productData) {
            res.render('admin/editproduct', {images,product: productData, categoryData: categoryData });
        }
        else {
...
const editProduct = async (req, res) => {
    try {
        ...
        console.log(oldPhotosArray,"imageeeeeeeeeesssssss");
...
const deleteProdImage = async (req, res) => {
    try {
        ...
        productData.image.splice(productData.image.indexOf(image), 1);
...
const deleteimage = async (req, res) => {
  ...
  console.log(productId,"firsttttttttttttttttt");
      const addressId = req.body.imageid;
      console.log(addressId,"secondddddddddddddddddddd");
  
      const addressData = await Product.findOneAndUpdate(
        { _id: imageid },
        { $pull: { cart: { imageid: req.body.imageid } } },
        { new: true }
      );
